...
def build_rating_list(name_to_rating, names_final):
    ...
    for name in names_final:
        list_of_ratings.append((name_to_rating[name],name))
    return sorted(list_of_ratings, key=lambda rating: rating[0], reverse=True)
...
def filter_by_cuisine(names_matching_price, cuisine_to_names, cuisines_list):
    ...
    """
    restaurants = []
    for cusine in cuisines_list:
        for retaurant in cuisine_to_names[cusine]:
            if retaurant in names_matching_price:
                restaurants.append(retaurant)
...
def read_restaurants(file):
    ...
    for i in range(0,len(lines),5):
        ...
        for x in lines[i+3].split(','):
            if x not in cuisine_to_names:
                cuisine_to_names[x] = []
                
            cuisine_to_names[str(x)].append(lines[i])
